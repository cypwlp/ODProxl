...
namespace ODProxl.ViewModels.Pages
{
    public class OnnxModelMSPageViewModel : BindableBase, INavigationAware
    {
        ...
        public OnnxModelMSPageViewModel(IDataService dataService, IOnnxModelAnalyzer onnxModelAnalyzer, HttpClient httpClient)
        {
            ...
            _sharedHttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(byteArray));
        ...
        private async Task LoadModelsFromServerAsync()
        {
            ...
            try
            {
                ...
                var regex = new Regex(@"(\d{4}/\d{1,2}/\d{1,2})\s+(\d{1,2}:\d{2})\s+(\d+)\s+<A\s+HREF=""([^""]+)"">([^<]+)</A>", RegexOptions.IgnoreCase | RegexOptions.Multiline);
                ...
                foreach (Match match in matches)
                {
                    ...
                    if (!fileName.EndsWith(".onnx", StringComparison.OrdinalIgnoreCase)) continue;
                    string fullUrl = relativeUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase) ? relativeUrl : new Uri(new Uri(_baseUrl), relativeUrl).ToString();
                    long.TryParse(sizeStr, out long sizeBytes);
                    DateTime lastModified = DateTime.Now;
                    if (DateTime.TryParse($"{dateStr} {timeStr}", out DateTime parsed)) lastModified = parsed;
        ...
        public async Task SetEnabledModelAsync(FileSystemItem selectedItem)
        {
            ...
            try
            {
                ...
                {
                    if (item != selectedItem && item.IsEnabled) item.IsEnabled = false;
                }
        ...
        private async Task SaveSelectedModelAsync(FileSystemItem selectedItem)
        {
            ...
            try
            {
                using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (!File.Exists(localModelPath) || new FileInfo(localModelPath).Length == 0)
                    {
                    ...
                    if (!string.IsNullOrWhiteSpace(raw))
                    {
                        ...
                        if (resultList == null)
                        {
                            ...
                            foreach (var part in parts)
                            {
                                ...
                                if (keyValue.Length == 2 && int.TryParse(keyValue[0].Trim(), out int suffix))
                                {
                                    string className = keyValue[1].Trim();
                                    if (className.StartsWith("\"") && className.EndsWith("\"")) className = className.Substring(1, className.Length - 2);
                                    else if (className.StartsWith("'") && className.EndsWith("'")) className = className.Substring(1, className.Length - 2);
                                    if (className.EndsWith("}")) className = className.TrimEnd('}');
                                    resultList.Add(new ClassInfo { Suffix = suffix, ClassName = className });
                                }
                            }
                        }
                    }
                    ...
                    // 上傳 classes.txt
                    string classesContent = resultList != null && resultList.Any() ? GenerateClassesFileContent(resultList) : "# No classes";
                    ...
                    string mergeSql = @"
                        ...
                        WHEN MATCHED THEN UPDATE SET model_name = @ModelName, model_path = @ModelPath
                        WHEN NOT MATCHED THEN INSERT (model_userAccount, model_name, model_path) VALUES (@LoginName, @ModelName, @ModelPath)
                    ...
                    if (!int.TryParse(resultId, out int modelId)) throw new Exception("無法取得模型 ID");
                    ...
                    {
                        string insertSql = @"INSERT INTO sys_model_classes (class_model_id, class_suffix, class_name) VALUES (@ModelId, @ClassSuffix, @ClassName)";
            ...
            {
                Debug.WriteLine($"❌ 啟用模型失敗: {ex.Message}");
            }
        }

        private string GenerateClassesFileContent(IEnumerable<ClassInfo> classInfos)
        {
            if (classInfos == null || !classInfos.Any()) return string.Empty;
        ...
        private async Task GetUserEnableModelAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(result))
            {
                ...
                if (matchedItem != null) matchedItem.IsEnabled = true;
            }
        }
        ...
        {
            Items.Clear();
            var filtered = string.IsNullOrWhiteSpace(SearchText) ? _allItems : _allItems.Where(i => i.Name.Contains(SearchText, StringComparison.OrdinalIgnoreCase)).ToList();
